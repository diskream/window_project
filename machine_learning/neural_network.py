...
class NNView(tk.Tk):
    def __init__(self, entry):
        ...
        save_frm = ttk.Frame(self.left_frm)
        save_frm.pack(side=tk.BOTTOM)
        ...
        ttk.Label(central_selection_frm, text='Пожалуйста, выберите решаемую задачу:').grid(row=0, column=0,
                                                                                            columnspan=2)
        ...
        forecasting_frm = ForecastingFrame(self.left_frm, self, self.entry, self.pd_data)
        ...
        ttk.Label(save_frm, text='Название модели:').pack(side=tk.LEFT, **pad)
        self.model_name_ent = ttk.Entry(save_frm)
        self.model_name_ent.pack(side=tk.LEFT, **pad)
        ttk.Button(save_frm, text='Сохранить', command=self.save_model).pack(side=tk.LEFT, **pad)
        self.model_save_lbl = ttk.Label(save_frm, text='')
    ...
    def get_model(self):
        ...
        cur = conn.cursor()
        try:
            path = deserialize(cur.execute(f'SELECT bin_file FROM Models WHERE name = "{model}"').fetchone()[0])
    ...
    def save_model(self):
        ...
        if name == '':
            self.model_save_lbl.configure(text='Введите название!')
            return
        try:
            path = f'other/data/'
            save_model(self.entry, self.model, accuracy=self.score[-1], name=name, path=path)
...
class ClassificationFrame(tk.Frame):
    def __init__(self, parent, master, entry, pd_data):
        ...
        layer_frm = ttk.LabelFrame(self.layer_configure_frm, text='Настройка слоев Dense')
...
class ForecastingFrame(ClassificationFrame):
    def __init__(self, parent, master, entry, pd_data):
        tk.Frame.__init__(self, parent)
        self.master = master
        self.parent = parent
        ...
        configuration_frm = tk.Frame(self.layer_configure_frm)
        configuration_frm.pack(side=tk.LEFT, anchor=tk.N, padx=5, pady=15)
        layer_frm = ttk.LabelFrame(self.layer_configure_frm, text='Настройка слоев LSTM')
        layer_frm.pack(side=tk.RIGHT, anchor=tk.N, padx=5, pady=5)
    ...
    def fit(self):
        ...
        x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.3)
        model = Sequential()
        model.add(LSTM(units=int(self.layer_unit_ent_1.get()),
                       activation=self.layer_activation_cb_1.get(),
                       input_shape=(x_train.shape[1], )))
        if int(self.layer_amount_sb.get()) == 2:
            model.add(LSTM(units=int(self.layer_unit_ent_2.get()),
                           activation=self.layer_activation_cb_2.get()))
        model.add(Dense(units=int(self.layer_unit_ent_end.get()),
                       activation=self.layer_activation_cb_end.get()))
        model.compile(**self.get_compile())
