...
class ClusteringView(tk.Tk):
    def __init__(self, entry):
        ...
        self.pd_data5 = deserialize(self.entry.table_file)
        ...
        self.update_title()
        # Фреймы алгоритмов
        self.kmeans_frm = KMeansFrame(self.left_frm, self.entry, self.pd_data, self.pd_data2, self.pd_data3, self.pd_data5)
        self.dbscan_frm = DBSCANFrame(self.left_frm, self.entry, self.pd_data, self.pd_data2, self.pd_data3, self.pd_data5)
...
class WorkWithModel(ttk.LabelFrame):
    ...
    def predict(self):
        list1 = self.model.feature_names_in_
        list1 = list(list1)
        print(list1)
        print(type(list1))
        ...
        for name in self.data.columns:
            if (type(self.data[name][1]) == str):
                str_col_names.append(name)
        le = LabelEncoder()
        for name in str_col_names:
            self.data[name] = le.fit_transform(self.data[name])
        # print(self.data)
        data = self.data[list(list1)]


        if set(data.columns) == set(list1):
            data['Prediction'] = self.model.fit_predict(data)
...
class KMeansFrame(tk.Frame):
    def __init__(self, parent, entry, pd_data, pd_data2, pd_data3, pd_data5):
        ...
        ttk.Label(self.clf_conf_frm, text='normalize').grid(row=6, column=0, columnspan=1, **pad)
        self.algorithm_norm = ttk.Combobox(self.clf_conf_frm, values=['0', '1'], **ent_options)
        self.algorithm_norm.grid(row=6, column=1, columnspan=1, **pad)
        self.algorithm_norm.current(0)
        CreateToolTip(self.algorithm_norm, text='Нормализация данных')
    ...
    def fit(self):
        ...
        norm = self.algorithm_norm.get()
        # print(norm)
        # print(type(norm))
        if norm == '1':
            # self.pd_data['Clusters'] = self.alg.fit_predict(self.pd_data)
            self.pd_data5 = preprocessing.MinMaxScaler().fit_transform(self.pd_data[self.pd_data.columns].values)
            kmeans = self.alg.fit(self.pd_data5)
            # print(kmeans.labels_)
            # self.pd_data2['Clusters'] = self.pd_data['Clusters']
            self.pd_data['Clusters'] = kmeans.labels_
            self.pd_data2['Clusters'] = kmeans.labels_
            self.isFitted = True
...
class DBSCANFrame(tk.Frame):
    def __init__(self, parent, entry, pd_data, pd_data2, pd_data3, pd_data5):
        ...
        ttk.Label(self.clf_conf_frm, text='metric').grid(row=4, column=1, columnspan=1, **pad)
        self.algorithm_cb = ttk.Combobox(self.clf_conf_frm, values=['euclidean', 'cosine', 'manhattan'], **ent_options)
        self.algorithm_cb.grid(row=5, column=1, columnspan=1, **pad)
        self.algorithm_cb.current(0)
        self.default_params['metric'] = self.algorithm_cb
        CreateToolTip(self.algorithm_cb, text='Метрика для подсчета расстояния\n'
                                              'между объектами')
        btn_pack = {
            'side': tk.LEFT,
            'padx': 10,
